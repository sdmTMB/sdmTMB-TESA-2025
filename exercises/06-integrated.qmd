...
-   Gain exposure to combining survey data from two surveys with different catchabilities
-   Gain familiarity working with offsets
-   Gain exposure to working with spatially varying coefficients for catchability
...
```{r}
dat <- readRDS(here::here("data/dogfish-hbll-iphc.rds"))
head(dat)
table(dat$survey)
```
...
We are effectively converting all observations into HBLL observations. This is the key part of accounting for gear catchability when combining data from different gear types. Importantly, by fitting an overall catchability effect, we are assuming that the gear types have approximately the same length or age selectivity.
...
```{r}
grid_one <- readRDS(here::here("data/hbll-s-grid.rds"))
...
-   If not, how might we do that? (2-3 ways I'm aware of)
